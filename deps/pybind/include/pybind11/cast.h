...
/// interoperability is added into pybind.
...
// Trampoline use: Having a pybind::cast with an invalid reference type is going to
// static_assert, even though if it's in dead code, so we provide a "trampoline" to pybind::cast
// that only does anything in cases where pybind::cast is valid.
template <typename T>
...
#ifndef NDEBUG
    if (!PyGILState_Check()) {
        pybind11_fail("pybind::object_api<>::operator() PyGILState_Check() failure.");
    }
#endif
