...
class ResidualCodecGenerate:
    Embeddings = ResidualEmbeddings

    def __init__(self, vec_dim, nbits, use_gpu, centroids, avg_residual=None, bucket_cutoffs=None, bucket_weights=None):
        self.use_gpu = use_gpu

        ResidualCodecGenerate.try_load_torch_extensions(self.use_gpu)

        if self.use_gpu > 0:
            self.centroids = centroids.cuda().half()
        else:
            self.centroids = centroids.float()
        self.dim, self.nbits = vec_dim, nbits
    ...
    @classmethod
    def load(cls, index_path, vec_dim, nbits, use_gpu):
        ...
        return cls(vec_dim=vec_dim, nbits=nbits, use_gpu=use_gpu,
                   centroids=centroids, avg_residual=avg_residual, bucket_cutoffs=bucket_cutoffs,
    ...
    def compress(self, embs):
        ...
        return ResidualCodecGenerate.Embeddings(codes, residuals)
    ...
    def binarize(self, residuals):
        ...
        if self.use_gpu:
            residuals_packed = ResidualCodecGenerate.packbits(residuals.contiguous().flatten())
        else:
    ...
    def decompress(self, compressed_embs: Embeddings):
        ...
        for codes_, residuals_ in zip(codes.split(1 << 15), residuals.split(1 << 15)):
            if self.use_gpu:
                ...
                centroids_ = ResidualCodecGenerate.decompress_residuals(
                    residuals_,
                    self.bucket_weights,
                    self.reversed_bit_map,
                    self.decompression_lookup_table,
