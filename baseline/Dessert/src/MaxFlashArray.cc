...
template <typename LABEL_T>
uint64_t MaxFlashArray<LABEL_T>::addDocument(const EmbeddingArray& batch, const uint32_t& n_thread) {
  ...
  const std::vector<uint32_t> hashes = hash(batch, n_thread);
...
template <typename LABEL_T>
std::pair<std::vector<float>, size_t> MaxFlashArray<LABEL_T>::getDocumentScores(
    const EmbeddingArray& query,
    const std::vector<uint32_t>& documents_to_query, const uint32_t& n_thread) const {
  const std::vector<uint32_t> hashes = hash(query, n_thread);
  uint32_t num_vectors_in_query = query.request().shape[0];

  std::vector<float> result(documents_to_query.size());
  std::vector<size_t> n_score_refine_l(documents_to_query.size());

#pragma omp parallel default(none) \
    shared(result, documents_to_query, hashes, query, num_vectors_in_query, n_score_refine_l) num_threads(n_thread)
  {
...
#pragma omp for
    for (uint64_t i = 0; i < result.size(); i++) {
      ...
      std::pair<float, size_t> res_pair = _maxflash_array.at(flash_index)
                      ->getScore(hashes, num_vectors_in_query, buffer, _collision_count_to_sim);
        result[i] = res_pair.first;
        n_score_refine_l[i] = res_pair.second;
  ...
  const size_t n_score_refine = std::accumulate(n_score_refine_l.begin(), n_score_refine_l.end(), size_t(0));

  return std::make_pair(result, n_score_refine);
}

template <typename LABEL_T>
std::vector<uint32_t> MaxFlashArray<LABEL_T>::hash(const EmbeddingArray& batch, const uint32_t& n_thread) const {
  auto buffer = batch.request();
  uint32_t num_vectors = buffer.shape[0];
