...
class MaxFlashArray {
 public:
  ...
  uint64_t addDocument(const EmbeddingArray& batch, const uint32_t& n_thread);

  std::pair<std::vector<float>, size_t> getDocumentScores(
 ...
 private:
  std::vector<uint32_t> hash(const EmbeddingArray& batch, const uint32_t& n_thread) const;
