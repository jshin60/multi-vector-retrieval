...
class DocSearch {
 public:
  ...
  // and we updated it.
  bool addDocument(const EmbeddingArray& doc_embeddings, const std::string& doc_id, const uint32_t &n_thread) {
    std::vector<uint32_t> centroid_ids = getNearestCentroids(doc_embeddings, 1, n_thread);
    return addDocumentWithCentroids(doc_embeddings, doc_id, centroid_ids, n_thread);
  }
  ...
  bool addDocumentWithCentroids(const EmbeddingArray& doc_embeddings,
    ...
    uint32_t internal_id = _document_array->addDocument(doc_embeddings, n_thread);
  ...
  std::tuple<std::vector<std::string>, double, double, uint32_t, size_t> query(
      const EmbeddingArray& embeddings, uint32_t top_k,
      uint32_t num_to_rerank, const uint32_t& n_thread) {
    std::vector<uint32_t> centroid_ids =
        getNearestCentroids(embeddings, _nprobe_query, n_thread);
    return queryWithCentroids(embeddings, centroid_ids, top_k, num_to_rerank, n_thread);
  }

  std::tuple<std::vector<std::string>, double, double, uint32_t, size_t> queryWithCentroids(
      const EmbeddingArray& embeddings, const std::vector<uint32_t>& centroid_ids,
      uint32_t top_k, uint32_t num_to_rerank, const uint32_t& n_thread) {
    ...
    std::vector<uint32_t> top_k_internal_ids =
        frequencyCountCentroidBuckets(centroid_ids, num_to_rerank, n_thread);
    const double filter_time = time_record.get_elapsed_time_second();
    const uint32_t n_refine = top_k_internal_ids.size();

    time_record.reset();
    auto rank_doc_res =
        rankDocuments(embeddings, top_k_internal_ids, n_thread);
    const double rerank_time = time_record.get_elapsed_time_second();
    std::vector<uint32_t> reranked = rank_doc_res.first;
    const size_t n_score_refine = rank_doc_res.second;
    ...
    std::tuple<std::vector<std::string>, double, double, uint32_t, size_t> result_tuple(result, filter_time, rerank_time, n_refine, n_score_refine);
//    return result;
    return result_tuple;
  }
  ...
  std::pair<std::vector<uint32_t>, size_t> rankDocuments(
      const EmbeddingArray& query_embeddings,
      const std::vector<uint32_t>& internal_ids_to_rerank, const uint32_t& n_thread) const {
    ...
    std::pair<std::vector<float>, size_t> doc_scores_pair = _document_array->getDocumentScores(
        query_embeddings, internal_ids_to_rerank, n_thread);
    std::vector<float> document_scores = doc_scores_pair.first;
    const size_t n_score_refine = doc_scores_pair.second;
    ...
    return std::make_pair(permuted_ids, n_score_refine);
  }
  ...
  std::vector<float> getScores(
      const EmbeddingArray& query_embeddings,
      const std::vector<uint32_t>& internal_ids_to_rerank, const uint32_t& n_thread) const {
        return _document_array->getDocumentScores(query_embeddings, internal_ids_to_rerank, n_thread).first;
    }
 ...
 private:
  ...
  std::vector<uint32_t> getNearestCentroids(const EmbeddingArray& batch,
                                            uint32_t nprobe, const uint32_t& n_thread) const {
...
#pragma omp parallel for default(none) \
    shared(batch, eigen_result, nprobe, nearest_centroids, num_vectors) num_threads(n_thread)
  ...
  std::vector<uint32_t> frequencyCountCentroidBuckets(
      const std::vector<uint32_t>& centroid_ids, uint32_t top_k, const uint32_t& n_thread) {
    ...
    // use the argmax util_fastrag function. We negate a counts element when we have
