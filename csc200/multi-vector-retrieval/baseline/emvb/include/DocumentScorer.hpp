...
//        printf("after filter if\n");
    ...
    vector<numDocsType>
    compute_hit_frequency(vector<numDocsType> &candidate_documents, const size_t k_centroids) {
    ...
    inline vector<float> compute_ip_with_centroids(const float *queries, const numDocsType doc_id, size_t& n_doc_vq_score_refine) {
...
//        printf("after compute_score_by_column_reduction\n");
    ...
    second_stage_filtering(const float *queries_data, const globalIdxType q_start, const vector<numDocsType> &doc_ids,
                           const size_t n_documents, size_t& n_vq_score_refine) {
        ...
        for (size_t doc_idx = 0; doc_idx < doc_ids.size(); doc_idx++) {
            ...
            size_t n_doc_vq_score_refine = 0;
            auto centroid_distances = compute_ip_with_centroids(queries_data + q_start, doc_id, n_doc_vq_score_refine);
            n_vq_score_refine += n_doc_vq_score_refine;
