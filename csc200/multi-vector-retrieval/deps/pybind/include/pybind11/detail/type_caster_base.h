/*
    pybind/detail/type_caster_base.h (originally first part of pybind/cast.h)
...
public:
    ...
    /// This can only be used inside a pybind-bound function, either by `argument_loader`
    /// at argument preparation time or by `py::cast()` at execution time.
    PYBIND11_NOINLINE static void add_patient(handle h) {
        loader_life_support *frame = get_stack_top();
        if (!frame) {
            // NOTE: It would be nice to include the stack frames here, as this indicates
            // use of pybind::cast<> outside the normal call framework, finding such
    ...
    for (size_t i = 0; i < check.size(); i++) {
        ...
        if (it != type_dict.end()) {
            ...
            for (auto *tinfo : it->second) {
                ...
                // having a large number of immediate pybind-registered types seems fairly
...
/**
 * Gets a single pybind type info for a python type.  Returns nullptr if neither the type nor any
 * ancestors are pybind-registered.  Throws an exception if there are multiple bases--use
...
PYBIND11_NOINLINE detail::type_info *get_type_info(PyTypeObject *type) {
    ...
    if (bases.size() > 1) {
        pybind11_fail(
            "pybind::detail::get_type_info: type has multiple pybind-registered bases");
    }
    return bases.front();
}
...
PYBIND11_NOINLINE detail::type_info *get_type_info(const std::type_index &tp,
    ...
    if (throw_if_missing) {
        ...
        pybind11_fail("pybind::detail::get_type_info: unable to find type info for \""
                      + std::move(tname) + '"');
    }
    return nullptr;
}
 ...
 * if the given type (or ValueType, if omitted) is not a pybind base of the given instance.  If
...
instance::get_value_and_holder(const type_info *find_type /*= nullptr default in common.h*/,
    ...
    pybind11_fail("pybind::detail::instance::get_value_and_holder: `"
                  + get_fully_qualified_tp_name(find_type->type)
                  + "' is not a pybind base of the given `"
                  + get_fully_qualified_tp_name(Py_TYPE(this)) + "' instance");
#else
    pybind11_fail(
        "pybind::detail::instance::get_value_and_holder: "
        "type is not a pybind base of the given instance "
...
PYBIND11_NOINLINE void instance::allocate_layout() {
    ...
    if (n_types == 0) {
        pybind11_fail(
            "instance allocation failed: new instance has no pybind-registered base types");
    }
...
public:
    ...
    PYBIND11_NOINLINE bool load_impl(handle src, bool convert) {
        ...
        if (PyType_IsSubtype(srctype, typeinfo->type)) {
            ...
            // one simple (no MI) pybind class, or is an exact match, so the C++ instance is of
...
// registered with pybind, and this Animal is in fact a Dog).
//
...
public:
    ...
    static std::pair<const void *, const type_info *> src_and_type(const itype *src) {
        ...
        if (instance_type && !same_type(cast_type, *instance_type)) {
            ...
            // with pybind, we want to make the full derived object available.
