/*
    pybind/pytypes.h: Convenience wrapper classes for basic Python types
...
public:
    ...
    const handle &inc_ref() const & {
        ...
        if (m_ptr != nullptr && !PyGILState_Check()) {
            throw_gilstate_error("pybind::handle::inc_ref()");
        }
#endif
        Py_XINCREF(m_ptr);
        return *this;
...
#ifdef PYBIND11_ASSERT_GIL_HELD_INCREF_DECREF
        if (m_ptr != nullptr && !PyGILState_Check()) {
            throw_gilstate_error("pybind::handle::dec_ref()");
        }
#endif
        Py_XDECREF(m_ptr);
        return *this;
...
private:
    ...
    void throw_gilstate_error(const std::string &function_name) const {
        fprintf(
            ...
            "https://pybind.readthedocs.io/en/stable/advanced/"
...
#    endif
    ...
    void restore() {
        if (m_restore_called) {
            pybind11_fail("Internal error: pybind::detail::error_fetch_and_normalize::restore() "
...
inline std::string error_string() {
    return error_fetch_and_normalize("pybind::detail::error_string").error_string();
}
...
public:
    ...
    error_already_set()
        : m_fetched_error{new detail::error_fetch_and_normalize("pybind::error_already_set"),
...
// type, the value goes through pybind::cast(obj) to convert it to an `object`.
...
public:
    ...
    PYBIND11_DEPRECATED(
        "Use of obj.attr(...) as bool is deprecated in favor of pybind::hasattr(obj, ...)")
    explicit
