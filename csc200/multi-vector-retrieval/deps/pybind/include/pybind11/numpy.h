/*
    pybind/numpy.h: Basic NumPy support, vectorize() wrapper
...
#define DECL_NPY_API(Func) api.Func##_ = (decltype(api.Func##_)) api_ptr[API_##Func];
        ...
        if (api.PyArray_GetNDArrayCFeatureVersion_() < 0x7) {
            pybind11_fail("pybind numpy support requires numpy >= 1.7.0");
        }
...
protected:
    ...
    static PyObject *raw_array(PyObject *ptr, int ExtraFlags = 0) {
        if (ptr == nullptr) {
            PyErr_SetString(PyExc_ValueError, "cannot create a pybind::array from a nullptr");
            return nullptr;
        }
...
protected:
    ...
    static PyObject *raw_array_t(PyObject *ptr) {
        if (ptr == nullptr) {
            PyErr_SetString(PyExc_ValueError, "cannot create a pybind::array_t from a nullptr");
            return nullptr;
        }
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        for (auto shape_iter = buffers[i].shape.rbegin(); shape_iter != end;
            ...
            if (dim_size_out == 1) {
                ...
                pybind11_fail("pybind::vectorize: incompatible size/dimension of inputs!");
            }
        }
    }
...
private:
    ...
    static_assert(
        NVectorized >= 1,
        "pybind::vectorize(...) requires a function with at least one vectorizable argument");
