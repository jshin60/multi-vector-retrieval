...
// This file mimics a DSO that makes pybind calls but does not define a PYBIND11_MODULE,
// so that the first call of cross_module_error_already_set() triggers the first call of
// pybind::detail::get_internals().
...
void interleaved_error_already_set() {
    ...
    // The error_already_set dtor triggers a pybind::detail::get_internals()
    // call via pybind::gil_scoped_acquire.
    if (PyErr_Occurred()) {
        throw py::error_already_set();
    }
}
