...
def test_error_already_set(msg):
    ...
    assert (
        msg(excinfo.value)
        == "Internal error: pybind::error_already_set called while Python error indicator not set."
    )
...
def test_cross_module_exceptions(msg):
    ...
    assert str(excinfo.value) == "pybind value error"

    with pytest.raises(TypeError) as excinfo:
        cm.throw_pybind_type_error()
    assert str(excinfo.value) == "pybind type error"
...
# This can often happen if you wrap a pybind class in a Python wrapper
...
def test_flaky_exception_failure_point_init():
    ...
    assert lines[:3] == [
        "pybind::error_already_set: MISMATCH of original and normalized active exception types:"
...
def test_error_already_set_double_restore():
    ...
    assert str(excinfo.value) == (
        "Internal error: pybind::detail::error_fetch_and_normalize::restore()"
