...
def test_cpp_function_roundtrip():
    ...
    assert (
        m.test_dummy_function(m.dummy_function) == "matches dummy_function: evaluation(1) = 2"
    )
    assert (
        m.test_dummy_function(m.roundtrip(m.dummy_function))
        == "matches dummy_function: evaluation(1) = 2"
    )
    assert (
    ...
    assert (
        ...
        == "can't convert to function pointer: evaluation(1) = 3"
    )
